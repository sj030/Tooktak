...
import * as writable from "xlsx";

const CHUNK_SIZE = 512 * 1024 * 1024; // 512MB 단위 청크 크기
export function getFileZip(filePaths) {
    return axiosInstance.post('file/zip', filePaths)
}

export async function getDownload(zipId, fileSize, setProgress) {
    let downloadedSize = 0;
    if (fileSize <= CHUNK_SIZE) {
        const response = await axiosInstance.get(zipId, {responseType: 'blob'});
        ...
        while (downloadedSize < fileSize) {
            ...
            const response = await axiosInstance.get(zipId, config);
            await writable.write(response.data);
